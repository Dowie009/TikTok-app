...
# --- 1. アプリの設定 ---
st.set_page_config(page_title="アニ無理 制作ノート", layout="wide")
...
if finished_count > 0:
    ...
    deadline_text = f"{last_stock_date.strftime('%m/%d')} {last_stock_wday} まで"
    sub_text = "投稿可能！✨"
else:
    deadline_text = "在庫なし"
    sub_text = "撮影頑張りましょう！"
...
with col1:
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "タイトル": st.column_config.TextColumn(width="medium"), # ここを変更！
...
with col2:
    ...
    for idx, row in edited_df.iterrows():
        ...
        label = f"{status_mark} {row['公開予定日'].strftime('%m/%d')} {row['曜日']} : {display_title}"
    ...
    if new_text != current_text:
        ...
        st.toast(f"No.{selected_row['No']} の台本を保存しました！", icon="💾")
        st.rerun()